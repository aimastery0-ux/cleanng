...
function StarRow({ avg, count }: { avg: string; count: number }) {
  ...
  return (
    <span className="flex items-center gap-1 text-small text-grey-mid">
      <span className="flex">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className={`h-3 w-3 ${i < n ? "fill-orange text-orange" : "fill-border text-border"}`} />
        ))}
      </span>
  ...
  return (
    ...
    >
      <div className="shrink-0">
        {cleaner.avatar_url ? (
          <img src={cleaner.avatar_url} alt={cleaner.full_name}
            className="w-16 h-16 rounded-full object-cover border border-border" />
        ) : (
          <div className="w-16 h-16 rounded-full bg-bg-alt flex items-center justify-center text-2xl">👤</div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {cleaner.is_verified && <Badge variant="success" className="text-xs shrink-0">Verified</Badge>}
              {cleaner.is_featured && <Badge variant="orange" className="text-xs shrink-0">Featured</Badge>}
            </div>
            <StarRow avg={cleaner.rating_avg} count={cleaner.rating_count} />
          </div>
          <div className="text-right shrink-0">
            <p className="font-bold text-orange">₦{Number(cleaner.base_hourly_rate).toLocaleString()}</p>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {cleaner.service_areas.slice(0, 3).map((a) => (
            <span key={a} className="text-caption bg-bg-alt px-2 py-0.5 rounded-pill">📍 {a}</span>
          ))}
        ...
        {cleaner.distance_km !== null && (
          <p className="text-caption text-grey-mid mt-1">
            <MapPin className="inline h-3 w-3 mr-0.5" />{cleaner.distance_km} km away
          </p>
        )}
      </div>
    </Link>
...
function MapView({
  ...
  if (!isLoaded) {
    return <div className="h-full bg-bg-alt flex items-center justify-center text-grey-mid">Loading map…</div>;
  }

  const selectedCleaner = results.find((r) => r.id === selected);

  // Only render markers for cleaners that have real coordinates
  const positioned = results.filter((r) => r.lat != null && r.lng != null);
  ...
  return (
    ...
    >
      ...
      {selected && selectedCleaner?.lat != null && (
        <InfoWindow
...
export default function SearchPage() {
  ...
  // lat/lng from Places selection — kept in local state, passed as query params
  ...
  const handlePlaceSelect = ({ address, lat: pLat, lng: pLng }: { address: string; lat: number; lng: number }) => {
    setMapCenter({ lat: pLat, lng: pLng });
    setParam({ area: address, lat: pLat, lng: pLng });
  };
  ...
  const clearFilters = () => setSearchParams({ sort: "featured" });
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-border bg-white sticky top-0 z-20 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-3 flex-wrap">
          ...
          {/* Google Places area input */}
          <PlacesAutocomplete
            value={area}
            onChange={(val) => setParam({ area: val })}
        ...
        {/* Expanded filters */}
        {filtersOpen && (
          <div className="max-w-7xl mx-auto mt-3 pt-3 border-t border-border flex flex-wrap items-end gap-4">
            <div>
            ...
            <div>
              ...
              <select value={availableDay} onChange={(e) => setParam({ available_day: e.target.value })}
                className="input text-small py-1.5">
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-small text-error hover:underline">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <p className="text-small text-grey-mid">
            {isLoading ? "Searching…" : (
              <>
                <span className="font-semibold text-black">{data?.count ?? 0}</span>{" "}
                cleaner{(data?.count ?? 0) !== 1 ? "s" : ""} found
          ...
          <>
            {isLoading ? (
              ...
              <div className="py-20 text-center">
                ...
                <p className="text-grey-mid mb-4">Try a different area or remove some filters.</p>
                {hasFilters && (
                  <button onClick={clearFilters} className="btn btn-outline text-sm">Clear all filters</button>
                )}
              </div>
            ) : (
              <div className="space-y-3">
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex items-center justify-center gap-2">
                ...
                <span className="text-small text-grey-mid px-3">Page {page} of {totalPages}</span>
          ...
          <div className="flex gap-4 h-[calc(100vh-200px)]">
            ...
            <div className="w-80 overflow-y-auto space-y-3 shrink-0">
              {isLoading ? (
                ...
                <div className="text-center py-8 text-grey-mid text-small">No cleaners match your filters.</div>
              ) : (
                results.map((c) => <CleanerCard key={c.id} cleaner={c} />)
              )}
            </div>
